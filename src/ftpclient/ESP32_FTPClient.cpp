...
void ESP32_FTPClient::OpenConnection() {
  ...
  char _loginResp[ sizeof(outBuf) ];
  GetFTPAnswer(_loginResp);
  // _isConnected reflects whichever command ran most recently, and some
  // servers (tnftpd included) reply successfully to SYST even when login
  // failed, silently masking a bad login. Capture the PASS result here so
  // a rejected login stays rejected.
  bool loginOk = (_loginResp[0] == '2');

  FTPdbgn("Send SYST");
  client.println(F("SYST"));
  GetFTPAnswer();

  if (!loginOk) _isConnected = false;
}
...
size_t ESP32_FTPClient::DownloadFileToFile(const char * filename, File &out) {
  FTPdbgn("Send RETR");
  ...
  char _resp[ sizeof(outBuf) ];
  GetFTPAnswer(_resp);
  // A "550" (or any non-1xx/2xx) reply means the file wasn't found or the
  // server refused - there's no data to follow, so don't wait on dclient.
  if (_resp[0] != '1' && _resp[0] != '2') {
    return 0;
  }

  unsigned long _m = millis();
  while (!dclient.available() && !dclient.connected() && millis() < _m + timeout) delay(1);

  size_t total = 0;
  unsigned char buf[512];
  unsigned long lastData = millis();
  while (dclient.connected() || dclient.available()) {
    int n = dclient.available();
    if (n > 0) {
      int toRead = n > (int)sizeof(buf) ? sizeof(buf) : n;
      int got = dclient.read(buf, toRead);
      if (got > 0) {
        out.write(buf, got);
        total += got;
        lastData = millis();
      ...
      if (millis() - lastData > timeout) break;
      delay(1);
    }
  }
