...
## Partition Layout Change (v0.8.0) — and Fix (v0.8.8)
...
v0.8.8 fixes this with `min_spiffs.csv` — two real 1.875MB OTA slots (up from the default's 1.25MB, and with real fallback safety this time), LittleFS at 128KB. Full details, including why the first attempt failed, in the README's "Partition Layout" section.

## Bug Fix (v0.8.8): mkdir + ls on SD card
...
## Bug Fix (v0.8.8): mv/cp with multiple glob matches to a non-existent destination
...
## Bug Fix (v0.8.8): nixfetch (and any single unpiped command) printed to the console instead of the web page
