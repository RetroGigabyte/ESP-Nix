...
- Shell prompt now shows the current user (`root@nix:/$`), reading from the existing `USER` variable rather than a hardcoded string - same for `whoami`. First piece of an eventual accounts/permissions framework; still one user, nothing access-controlled yet
...
## Partition Layout Change (v0.8.0) — and Fix (v0.8.9)
...
v0.8.9 fixes this with `min_spiffs.csv` — two real 1.875MB OTA slots (up from the default's 1.25MB, and with real fallback safety this time), LittleFS at 128KB. Full details, including why the first attempt failed, in the README's "Partition Layout" section.

## Bug Fix (v0.8.9): mkdir + ls on SD card
...
## Bug Fix (v0.8.9): mv/cp with multiple glob matches to a non-existent destination
...
## Bug Fix (v0.8.9): nixfetch (and any single unpiped command) printed to the console instead of the web page
